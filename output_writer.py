...
def write_new_csv(filename, list_of_dicts, statistics_dict):
    ...
    with open(results_path, mode="w") as file:
        writer = DictWriter(
            file, fieldnames=["cohort_name", "name", "github_username", "grade"]
        )
        writer.writeheader()
        for each_dict in list_of_dicts:
            writer.writerow(
                {
                    "cohort_name": each_dict["cohort_name"],
                    "name": each_dict["name"],
    ...
    with open(statistics_path, mode="w") as file:
        writer = DictWriter(
            file, fieldnames=["users", "median_tags", "median_repos", "median_pinned"]
        )
        writer.writeheader()
        writer.writerow(
            {
                "users": statistics_dict["users"],
